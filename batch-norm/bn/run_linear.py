...
l1_mean, l1_std, l2_mean, l2_std, loss_arr, model_type = train(model_type=ModelType.LINEAR, num_epochs=2)
l1_mean_bn, l1_std_bn, l2_mean_bn, l2_std_bn, loss_arr_bn, model_type_bn = train(model_type=ModelType.LINEAR_BN, num_epochs=2)
...
# plot for to see learning rate difference
plt.plot(loss_arr, 'y', label='%s loss' % model_type)
plt.plot(loss_arr_bn, 'r', label='%s loss' % model_type_bn)
plt.title('Loss: %s vs. %s' % (model_type, model_type_bn))
plt.legend(loc='upper right')
plt.savefig('imgs/loss.png')
plt.close()
